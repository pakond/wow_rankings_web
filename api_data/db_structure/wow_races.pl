...
my $sth = $db->prepare("DROP TABLE IF EXISTS `wow_races`");
$sth->execute() or die "imposible crear la tabla";
$sth->finish;

$sth = $db->prepare("CREATE TABLE `blizzardrankings`.`wow_races` ( `id` INT(2) NOT NULL, `name` VARCHAR(32) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL , `side` VARCHAR(32) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL, `mask` INT(32) NOT NULL) ENGINE = InnoDB CHARSET=utf8 COLLATE utf8_general_ci");
$sth->execute() or die "imposible crear la tabla";
$sth->finish;
...
foreach $indice (keys(@races)) {
	...
	print "$name\n";
	$sth = $db->prepare("INSERT INTO wow_races VALUES ('$json->{'races'}->[$indice]->{'id'}', '$name', '$side', '$json->{'races'}->[$indice]->{'mask'}')");
	$sth->execute() or die "imposible crear la tabla";
	$sth->finish;
}
