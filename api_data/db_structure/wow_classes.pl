...
my $contenido < io('https://eu.api.battle.net/wow/data/character/classes?locale=en_GB&apikey=5gdrgfafqdnkryj8tqafqxsdr4qamdcq');
...
my $sth = $db->prepare("DROP TABLE IF EXISTS `wow_classes`");
$sth->execute() or die "imposible borrar la tabla";
$sth->finish;

$sth = $db->prepare("CREATE TABLE `blizzardrankings`.`wow_classes` (`id` INT(2) NOT NULL, `name` VARCHAR(32) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL, `power_type` VARCHAR(32) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL, `mask` INT(32) NOT NULL) ENGINE = InnoDB CHARSET=utf8 COLLATE utf8_general_ci");
...
foreach my $indice (keys(@classes)) {
	...
	print "$name\n";
	$sth = $db->prepare("INSERT INTO wow_classes VALUES ('$json->{'classes'}->[$indice]->{'id'}', '$name', '$power', '$json->{'classes'}->[$indice]->{'mask'}')");
	$sth->execute() or die "imposible crear la tabla";
	$sth->finish;
}
